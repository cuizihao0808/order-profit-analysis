...
- 提供“补货批次计划”页面，展示各店铺按周分批发货的补货清单，并逐个 SKU / 整批标记已发货

页面左侧有可折叠的侧边栏，用来在“周订单利润”“重点关注产品”“补货批次计划”三个页面之间切换。
...
在左侧边栏切换到“补货批次计划”。数据来自 `src/data/replenishment/` 下的“补货清单_周批次版”CSV，每个店铺一个文件。

1. 顶部按钮切换店铺，按钮上的数字是该店铺清单里的 SKU 数
2. **汇总**：春节前计划件数、已发货件数与完成度、下一批（日期、还剩几天、待发件数）、采购金额、净重、节后首班参考
3. **发货批次**：每周一张卡片，显示件数、SKU 数、采购金额、净重和发货进度；状态有 待发货 / 已发 x/y / 已发货 / 逾期（批次日期已过但没发完）
4. **点一张批次卡片**选中该批次，下方出现操作栏：
   - **整批标记已发货**：把该批次所有 SKU 标记为已发货
   - **撤销本批已发货**：撤销该批次的已发货标记
   - **复制本批清单**：把该批次的 SKU / 品名 / ASIN / 件数复制到剪贴板（Tab 分隔，可直接粘贴到 Excel），末行是合计
5. **SKU 批次明细表**：每行一个 SKU，每个批次一列；点表格里的数字即可标记该 SKU 在该批次已发货（变绿并带 ✓），再点一次撤销。鼠标悬停可看到标记时间
6. 点行首 ▶ 展开日均需求、在途晚到残余缺口、判断说明和备注
7. “显示”筛选：全部SKU / 有待发 / 已发完 / 仅所选批次；右侧可按 SKU、品名、ASIN 搜索

表头和最后一行“每周合计”固定显示；合计始终按全部 SKU 计算，不受筛选影响。

**更新清单**：把新版 CSV 放进 `src/data/replenishment/`（替换旧文件或直接放入），点“刷新数据”。同一店铺有多个 CSV 时只用修改时间最新的那个，页面顶部会提示被忽略的文件。已发货标记按“店铺 + SKU + 批次日期”保存，新版清单里 SKU 和批次日期不变的标记会保留。
...
src/data/
  ...
  replenishment/
    *.csv                   补货清单_周批次版，每个店铺一个
    shipped.json            补货批次的已发货标记（首次标记时自动创建）
  products/
...
“补货批次计划”页面的数据源，UTF-8（可带 BOM）的 CSV，每个文件对应一个店铺（取“店铺”列的值，如 `TZH`）。按表头识别：

| 表头 | 说明 |
|------|------|
| `店铺` `SKU` | 必填；没有 SKU 列或没有批次列的文件会被跳过并提示 |
| `品名` `ASIN` `当前可用` `在途` `判断说明` `春节前合计` | 同名列 |
| `日均需求…` `…应急…` `…残余缺口` `…节后首班…` `…采购金额…` `…净重…` `备注…` | 按关键字匹配，表头里的日期等文字变化不影响识别 |
| `MM/DD` 或 `MM/DD(说明)` | 每周批次列，值为该 SKU 在该批次的发货件数；括号里的说明（如“仅现货”“春节前末班”）显示在批次卡片上 |

- SKU 以“每周”开头的行是每周合计，按名称识别件数、采购金额、净重、SKU 数，其中采购金额和净重用于批次卡片
- 批次表头只有月/日，年份以文件修改时间为准，月份回退时（12 月 → 1 月）自动跨年
- 店铺简称会按前缀匹配 `shops.json` 的店铺名（`TZH` → `TZH-主店二号`）用于显示；产品图片按 ASIN 从店铺主数据里取

CSV 自己解析，不经过 SheetJS，因为 SheetJS 会把 `10/10` 这类表头当成日期改写。

### replenishment/shipped.json
...
{
  "TZH": {
    "TR-AM9-2L": { "09/26": "2026-09-19T08:00:00.000Z" }
  }
}
```
...
| `opa:replenish-shop:v1` | 补货批次计划页当前选中的店铺 |
...
| `GET /api/replenishment` | 各店铺补货批次计划、已发货标记、文件告警 |
| `PUT /api/replenishment/shipped` | 标记或撤销已发货，body 为 `{ shop, entries: [{ sku, batch }], shipped }` |
...
- “补货批次计划”页面在 `src/components/ReplenishmentPlan.vue`
...
- 可测试的业务规则抽在 `src/lib/` 下：
  ...
  - `replenishmentPlan.js` 补货清单 CSV 解析、批次日期推断、发货进度汇总、已发货标记
